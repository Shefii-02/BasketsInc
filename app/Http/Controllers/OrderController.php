...
class OrderController extends Controller
{
    ...
    public function place_order(CheckoutFormRequest $request){
        ...
        if(session()->has('session_string') ){
            ...
            // 
            if( ($pay->getResponseCode() < 50 && strlen($refNum) > 5 && strlen($txnNum) > 5 && is_numeric($resCod)) || env('APP_URL') == 'https://basketsinc.indigitalapi.com')
            {
    ...
    function makePayment($paymeny_id,$grand_total,$basket, Request $request) 
    {
            ...
            $expiry_date    = $date;
            $crypt          = '7';
            $dynamic_descriptor='Basketsinc Order';
            $status_check   = 'false';
    ...
    function invoiceNumberGenerate(){
        ...
        return 'BSK'.date('ymd').sprintf('%04d', $ordercount+1);
    }
